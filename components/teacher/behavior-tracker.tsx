...
export function BehaviorTracker() {
  ...
  const [recentFeedback, setRecentFeedback] = useState<Record<number, { type: string; timestamp: Date }>>({})

  const handleSubmit = () => {
    if (!feedbackType || !feedbackCategory) {
      toast({
        title: "Missing Information",
        description: "Please select both a feedback type and category.",
        variant: "destructive",
      })
      return
    }
    ...
    if (selectedStudent) {
      setRecentFeedback({
        ...recentFeedback,
        [selectedStudent]: { type: feedbackType, timestamp: new Date() },
      })
    }
    ...
    toast({
      ...
      description: `Feedback for ${students.find((s) => s.id === selectedStudent)?.name} has been recorded.`,
    })
              ...
              {students.map((student) => (
                <TableRow key={student.id}>
                  ...
                  <TableCell className="text-right">
                    {recentFeedback[student.id] ? (
                      <Badge
                        variant="outline"
                        className={
                          recentFeedback[student.id].type === "positive"
                      ...
                      >
                        ...
                        {recentFeedback[student.id].type === "positive" ? "Positive" : "Needs Improvement"}
                      </Badge>
                    ) : (
                      <Badge
                        variant="outline"
                      ...
                      >
                        ...
                        {student.id % 2 === 0 ? "Positive" : "Needs Improvement"}
                      </Badge>
                    )}
                  </TableCell>
                  <TableCell className="text-right">
          ...
          {selectedStudent && (
            <div className="border rounded-lg p-4 mt-4">
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                <Button onClick={handleSubmit} disabled={!feedbackType || !feedbackCategory}>
                  Submit Feedback
                </Button>
              </div>
            </div>
